...
The OneKey framework requires the ad generation process (including the ad auction process) to produce an **Audit Log**.
Ultimately, the purpose of the Audit Log is to let the user collect the list of actors who have participated in the generation of marketing content, along with the electronic signature of each of those actors.

This requires the several actors involved to follow a common protocol to ensure that a complete Audit Log is eventually available to the user when the ad is displayed.

Typically, the actors are :
- a publisher,
- one or more SSPs,
- possibly some Ad Exchanges,
- and one or more DSPs.

In nutshell, the protocol's design goal are :
1. to get every actor to sign a data structure which uniquely identifies the ad impression,
2. to get all those signatures back to the publisher,
3. and to identify which signatures are relevant for the winning ad.

## Protocol data model

The protocol involves 4 types of elements:
* **Seeds**
...
A Seed expresses the usage of a user's Id and Preferences by a publisher for initiating an ad generation process for a set of placements on a page.

A Seed is unique, and includes:
- the publisher identity
- a set of Transaction ids (one for each ad placement)

It is electronically signed by the publisher.

### Transmission Requests

A Transmission Request is sent by an actor to share the a user's Id and Preferences to a receiving actor, so that the receiver may use them in the ad generation process.

A Transmission Request includes:
- a Seed 
- an array of parent Transmission Results that is chronogically ordered - from the oldest ancestor to the sending actor
    - the details of a Transmission Result are described below.

When an actor sends User Id and Preferences in a bid request, it must include a Transmission Request within it.

### Transmission Responses

A Transmission Response is sent back from a receiving actor to the actor who sent a transmission request.

A Transmission Response includes:
- the receiver Transmission Result
    - the details of a Transmission Result are described below.
- a tree of the child Transmission Responses if the receiver forwarded the Id and Preferences to other actors.

An actor must answer with a Transmission Reponse in the bid response only if it actually used the User Id and Preferences. Typically, this means they were used to generate content, or that they were forwarded to at least one other actor.

### Transmission Results 

A Transmission Result is generated by an actor to acknowledge that it has received and used the User Id and Preferences.

A Transmission Response includes:
- the receiver identity
- the status of the Transmission
- optionally, one or several associations between a Transaction Id and a Content Id, if the receiver itself generated content for some of the placements

It is electronically signed by the receiver.

### Audit Log chain

At the end of the ad generation process, the publisher receives a full tree containing all the Transmission Results that were generated for the ad placements mentionned in the Seed.

The Audit Log for a specific ad placement can be extracted from that tree by taking all the Transmission Results in the path to the Content Id of the winning ad.

Note that if several fragments of content were generated for a single ad by different actors, it's possible to extract all the Transmission Results in the path to all the Content Ids.

### Signatures

Seeds and Transmission Results are signed.

The signature algorithm is detailed in [security-signatures.md](security-signatures.md#signing-an-object).

For both type of object, the payload that must be signed is described in the `source`field documentation of the object:
- [seed.md](./model/seed.md)
- [transmission-result.md](./model/transmission-result.md)
