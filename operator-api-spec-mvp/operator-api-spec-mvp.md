...
- **Identifiers** (ID) are pseudonymous identifiers that are stored by Prebid SSO. 
  - In a near future, Prebid SSO will allow to store multiple IDs of different types, but there will always be at least one, which is the "**primary id**": the Prebid ID, **generated by an operator**.
...
For more details and examples, see [DSP API design](https://github.com/criteo/addressable-network-proposals/mvp-spec/dsp-api.md).
...
⚠️ TODO: detail the exact "message signature" algorithm. Need to detail what is signed, in what format
(ex: how to sort keys of a JSON file, how to order elements of an array, etc).
Ideally signatures should be identical regardless if called on `/json` or `/redirect`
